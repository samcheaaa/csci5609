<script lang="ts">
  ...
  function barOpac(genre: string) {
    if (!selectedGenre) return 1;
    return selectedGenre === genre ? 1 : 0.3
  }
</script>
      ...
      {#each Object.entries(genreNums) as [genre, cnt]}
        <g class={genre}>
          ...
          <rect
            ...
            fill="#449900"
            class="bar"
            opacity={barOpac(genre)}
            onmouseenter={() => (selectedGenre = genre)}
            onmouseleave={() => (selectedGenre = undefined)}
          />
